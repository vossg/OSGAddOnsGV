...
namespace OSG
{
	...
	void CpuClipmapRenderer::onRender( const ClipmapRenderParameters& renderParameters )
	{
		...
		{
			renderParameters.globalTextureObj->activate( renderParameters.renderAction->getDrawEnv() );
			renderParameters.globalTextureEnv->activate( renderParameters.renderAction->getDrawEnv() );
		}
		else if( renderParameters.heightColorTexture != NullFC )
		{
			// todo: use a different pixel shader here
			useDependentTextureLookup = true;
			renderParameters.heightColorTexture->activate( renderParameters.renderAction->getDrawEnv() );
		}
		...
		if( renderParameters.globalTextureObj != NullFC )
		{
			renderParameters.globalTextureObj->deactivate( renderParameters.renderAction->getDrawEnv() );
			renderParameters.globalTextureEnv->deactivate( renderParameters.renderAction->getDrawEnv() );
		}
		else if( renderParameters.heightColorTexture != NullFC )
		{
			renderParameters.heightColorTexture->deactivate( renderParameters.renderAction->getDrawEnv() );
		}
