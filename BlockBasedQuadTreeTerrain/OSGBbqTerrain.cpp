...
const BbqRenderStatistics &BbqTerrainEngineBase::getRenderStatistics(void) const
{
...
bool BbqTerrainEngineBase::initialize(BbqDataSource *pDataSource, 
...
void BbqTerrainEngineBase::update(const Vec3f &vViewerPosition)
{
//    Profile( terrainUpdate );
    ...
    if(_pRootNode)
    {
        ...
        {
            BbqBaseNode *node = _vTraversalStack.back();
...
void BbqTerrainEngineBase::render(const BbqRenderOptions &oOptions)
{
...
bool BbqTerrainEngineBase::prefetchNodeData(void)
{
...
//    MutexLock lock( treeAccessMutex_ );
    ...
    BbqBaseNode *pLoadNode = _qLoadQueue.front();
    ...
    if(!allocateChildren(pLoadNode))
    {
        ...
        BbqBaseNode *pUnloadNode = _qUnloadQueue.front();
...
void BbqTerrainEngineBase::requestLoad(
          BbqBaseNode *pNode, 
...
void BbqTerrainEngineBase::requestUnload(
          BbqBaseNode *pNode, 
    const Vec3f       &vViewerPosition)
{
    assert(pNode         );
...
void BbqTerrainEngineBase::calculatePriority(
          BbqBaseNode *pNode, 
    const Vec3f       &vViewerPosition)
{
    assert(pNode);
    ...
    BbqBaseNode *pParent = pNode->parent;
...
void BbqTerrainEngineBase::initializeNode(BbqBaseNode *pNode, 
                                          BbqNodeId    id, 
                                          Int32        x0,
                                          Int32        y0, 
                                          Int32        x1,
...
bool BbqTerrainEngineBase::allocateChildren(BbqBaseNode *pNode)
{
    assert(pNode              );
    assert(pNode->id >= 0     );
...
void BbqTerrainEngineBase::freeChildren(BbqBaseNode *pNode)
{
    assert(pNode);
...
void BbqTerrainEngineBase::checkTreeConsistency(BbqBaseNode *pNode)
{
    assert(pNode                                 );
    assert(pNode->id >= 0                        );
...
bool BbqTerrainEngineBase::loadChildren(BbqBaseNode *pNode)
{
    assert(pNode                 );
